<template>
  <div class="min-h-screen bg-gray-100">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h2 class="text-2xl font-bold text-gray-900 mb-6">卡密管理</h2>
...
const fetchKamis = async () => {
  try {
    ...
    if (filter.value.user_id) {
      params.append('user_id', filter.value.user_id);
    }

    const response = await fetch(`${API_ENDPOINTS.admin.kamis}?${params.toString()}`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
    });
...
const deleteKami = async (id: number) => {
  if (confirm('确定要删除这个卡密吗？')) {
    try {
      const response = await fetch(API_ENDPOINTS.admin.kami(id), {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
